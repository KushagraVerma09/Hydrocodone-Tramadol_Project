...
## real and independent. It is called namespaced, ggtext::element_markdown(), so
## it works off the INSTALLED package and does not depend on ggtext having been
## attached -- one less thing that can go stale in a long-running session. The catch that carries over from the plotmath version:
...
## facet_wrap(~ .strip, labeller = label_value) with strip.text = ggtext::element_markdown().
...
make_figure1 <- function(data, yvar, ylab, tag,
  ...
  if (stats_mode == "header") {
    ...
    p <- p + facet_wrap(~ .strip, scales = "fixed", labeller = label_value) +
      theme(strip.text = ggtext::element_markdown(face = "plain", size = FIG_STRIP_SIZE,
                                          lineheight = 1.15,
